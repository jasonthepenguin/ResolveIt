...
@onready var emoji_controller: Sprite3D = $EmojiManager
...
func _ready():
	_set_looped_animations()
	randomize()
	await get_tree().create_timer(0.1).timeout
	move_to_random_location()
	play_animation(AnimationState.IDLE)

func _set_looped_animations():
	animation_player.get_animation("Unarmed_Idle").loop_mode = Animation.LOOP_LINEAR
	...
	animation_player.get_animation("Jump_Full_Long").loop_mode = Animation.LOOP_LINEAR
	#animation_player.get_animation("PickUp").loop_mode = Animation.LOOP_LINEAR
	#animation_player.get_animation("Use_Item").loop_mode = Animation.LOOP_LINEAR
	#animation_player.get_animation("Lie_Down").loop_mode = Animation.LOOP_LINEAR
	animation_player.get_animation("Sit_Floor_Idle").loop_mode = Animation.LOOP_LINEAR
...
func play_animation(anim_state: AnimationState):
	...
	match anim_state:
		AnimationState.IDLE:
			animation_player.play("Unarmed_Idle", 0.2)
		AnimationState.WALK:
			animation_player.play("Walking_A", 0.2)
		AnimationState.RUN:
			animation_player.play("Running_A", 0.2)
		AnimationState.JUMP:
			animation_player.play("Jump_Full_Long", 0.2)
		AnimationState.INTERACT:
			if emoji_controller.texture == emoji_controller.angry_Emoji:
				animation_player.play("PickUp", 0.2)
			else:
				animation_player.play("Use_Item", 0.2)
		AnimationState.LIE_IDLE:
			animation_player.play("Lie_Down", 0.2)
		AnimationState.SIT_FLOOR_IDLE:
			animation_player.play("Sit_Floor_Idle", 0.2)
